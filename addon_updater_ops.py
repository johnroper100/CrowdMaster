...
# simple popup for prompting checking for update & allow to install if
# available
    ...
    def execute(self, context):
        ...
        if updater.update_ready == True:
            res = updater.run_update(
                force=False, callback=post_update_callback)
...
def post_update_callback():
    ...
    if updater.verbose:
        print("Running post update callback")
...
# function for asynchronous background check, which *could* be called on
# register
def check_for_update_background(context):
    ...
    # if false, no update ready
    if updater.verbose:
        print("Running background check for update")
...
def register(bl_info):
    ...
    # updater.version_max_update = (9,9,9) # max install (<) will install
    # strictly anything lower
