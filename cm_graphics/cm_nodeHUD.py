...
def draw_object_status(obj, dpi_factor):
    ...
    text2 = "Show Debug Options is currently set to: {} | {}".format(
        preferences.show_debug_options, cm_hudText2)
    x = get_3d_view_tools_panel_overlay_width(
        bpy.context.area) + 20 * dpi_factor
    ...
    draw_text_custom(text1, x, y1, font_file1, "ON",
                     size=int(size1), color=(0.8, 0.5, 0.0, 1.0))
    draw_text_custom(text2, x, y2, font_file2, "OFF",
                     size=int(size2), color=(0.7, 0.7, 0.7, 1.0))
...
def register():
    ...
    draw_handler = bpy.types.SpaceNodeEditor.draw_handler_add(
        draw_hud, tuple(), "WINDOW", "POST_PIXEL")
