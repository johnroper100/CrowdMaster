...
# plausibly could be some other system
...
class Singleton_updater(object):
    ...
    def get_tags(self):
        ...
        if self._tags == None:
            ...
            if self.verbose:
                print("Only master branch found:", self._tags[0])
            ...
            self._tag_latest = self._tags[0]
            if self.verbose:
                print("Most recent tag found:", self._tags[0])
    ...
    def get_api(self, url):
        ...
        get = None
        # this can fail by self-created error raising
    ...
    def upack_staged_zip(self):
        ...
        # verify, is __file__ always valid?
    ...
    def set_updater_json(self):
        ...
        if os.path.isfile(jpath):
            with open(jpath) as data_file:
                ...
                if self._verbose:
                    print("Read in json settings from file")
        else:
            # set data structure
            self._json = {
                "last_check": "",
    ...
    def start_async_check_update(self, now=False, callback=None):
        if self._async_checking == True:
            return
        if self._verbose:
            print("Starting background checking thread")
    ...
    def async_check_update(self, now, callback=None):
        ...
        if self._verbose:
            print("BG: Checking for update now in background")
